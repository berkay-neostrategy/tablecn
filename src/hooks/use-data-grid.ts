...
function useDataGrid<TData>({
  ...
  const searching = useStore(store, (state) => state.searchOpen);
  ...
  const onCellsPaste = React.useCallback(
    async (expandRows = false) => {
      ...
      try {
        ...
        // Get fresh rows after expansion
        const updatedRows = tableRef.current?.getRowModel().rows;
  ...
  const visualizer = useStore(store, (state) => state.visualizer);

  const onVisualizerOpenChange = React.useCallback(
    (open: boolean) => {
      store.setState("visualizer", {
        ...store.getState().visualizer,
        open,
      });
    },
    [store]
  ...
  const onVisualize = React.useCallback(() => {
    const currentState = store.getState();
    const { selectionRange, selectedCells } = currentState.selectionState;

    if (!selectionRange && selectedCells.size === 0) {
      toast.error("Please select cells to visualize");
      return;
    }
    ...
    if (selectionRange) {
      startRow = Math.min(selectionRange.start.rowIndex, selectionRange.end.rowIndex);
      endRow = Math.max(selectionRange.start.rowIndex, selectionRange.end.rowIndex);
      
      const startColIndex = columnIds.indexOf(selectionRange.start.columnId);
      const endColIndex = columnIds.indexOf(selectionRange.end.columnId);
      const minColIndex = Math.min(startColIndex, endColIndex);
      const maxColIndex = Math.max(startColIndex, endColIndex);

      selectedColumnIds = columnIds.slice(minColIndex, maxColIndex + 1);
    } else {
       const rows = Array.from(selectedCells).map(k => parseCellKey(k).rowIndex);
       if (rows.length === 0) return;
       startRow = Math.min(...rows);
       endRow = Math.max(...rows);
       
       const cols = Array.from(selectedCells).map(k => parseCellKey(k).columnId);
       const uniqueCols = Array.from(new Set(cols));
       selectedColumnIds = uniqueCols.filter(c => columnIds.includes(c));
    }
    ...
    for (let i = startRow; i <= endRow; i++) {
        ...
        const rowData: any = { _index: i }; // Internal index for reference
        let hasData = false;
        ...
        selectedColumnIds.forEach(colId => {
            ...
            if (cell) {
                ...
                rowData[colId] = !isNaN(numVal) && val !== "" && val !== null ? numVal : val;
                hasData = true;
            }
        });
    ...
    if (data.length === 0) {
        toast.error("No data found in selection");
        return;
    }
    ...
    (updater: Updater<ColumnOrderState>) => {
      ...
      const newColumnOrder =
        typeof updater === "function"
          ? updater(currentState.columnOrder)
          : updater;
  ...
  const tableState = React.useMemo<Partial<TableState>>(
    ...
    [propsRef, sorting, columnOrder, columnFilters, rowSelection]
  );
